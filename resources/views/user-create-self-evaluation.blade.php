                            ...
                            <div class="d-flex flex-row align-items-start w-100">
                                <div class="d-flex flex-column align-items-start part-form">
                                    <h5 class="fw-bold mt-2 mb-4">Objective Details</h5>
                                        ...
                                        <div class="d-flex align-items-start flex-column w-100 mb-3">
                                            <label class="label-form" for="date_debut">Date_debut</label>
                                            {{-- <p> {{ $objective->date_debut}} </p> --}}
                                        </div>

                                        <div class="d-flex align-items-start flex-column w-100 mb-3">
                                            <label class="label-form" for="date_fin" >Date_fin</label>
                                            {{-- <p> {{ $objective->date_fin}} </p> --}}
                                        </div>
                                ...
                                <div class="d-flex flex-column align-items-start part-form">
                                    <h5 class="fw-bold mt-2 mb-4">Evaluate Objective</h5>
                                @if(session('success'))
                                    <div class="alert alert-success">{{ session('success') }}</div>
                                @elseif(session('error'))
                                    <div class="alert alert-danger">{{ session('error') }}</div>
                                @endif
                                    <form action="{{ route('evaluate-form.store') }}" method="POST">
                                        @csrf

                                        <!-- On cache l'id de l'objectif ici -->
                                        <input type="hidden" name="objective_id" value="{{ $objective->id }}">

                                        <div class="d-flex align-items-start flex-column w-100 mb-3">
                                            <label for="score">Score</label>
                                            <input class="input-form-control" type="number" name="score" id="score" required>
                                        </div>

                                        <div class="d-flex align-items-start flex-column w-100 mb-3">
                                            <label for="comments">Comments</label>
                                            <textarea class="textarea-form-control" name="comments" id="comments" required></textarea>
                                        </div>
